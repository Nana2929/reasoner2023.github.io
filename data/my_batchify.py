...
class TagTestBatchify:
    def __init__(self, data, config, tag_num, shuffle=False):
        ...
        for x in data:
            ...
            candi_reason.append(
                self.get_candidate_tags(eval(x["reason_tag"]), config["candidate_num"])
            )
            candi_video.append(
                self.get_candidate_tags(eval(x["video_tag"]), config["candidate_num"])
            )
            candi_interest.append(
                self.get_candidate_tags(
                    eval(x["interest_tag"]), config["candidate_num"]
                )
            )
