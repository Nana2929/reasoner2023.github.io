...
class Trainer(object):
    ...
    def __init__(self, config, model, train_data, val_data):
        ...
        self.interest_weight = config['aspect_weight']
    ...
    def train(self, data):  # train mse+bce+l2
        ...
        while True:
            ...
            aspect_loss = self.model.calculate_aspect_loss(user, item, aspect_tag, aspect_label) * self.aspect_weight
    ...
    def valid(self, data):  # valid
        ...
        with torch.no_grad():
            while True:
                ...
                rating_loss = self.model.calculate_rating_loss(user, item, rating) * self.rating_weight
                aspect_loss = self.model.calculate_reason_loss(user, item, aspect_tag,
                                                               aspect_label) * self.aspect_weight
    ...
    def evaluate(self, model, data):  # test
        ...
        with torch.no_grad():
            while True:
                user, item, candi_aspect_tag, candi_video_tag, candi_interest_tag = data.next_batch()
                user = user.to(self.device)  # (batch_size,)
                item = item.to(self.device)
                reason_candidate_tag = candi_aspect_tag.to(self.device)
        ...
        # aspect_tag
        reason_p, reason_r, reason_f1 = evaluate_precision_recall_f1(self.top_k, data.positive_aspect_tag,
                                                                     reason_predict)
        reason_ndcg = evaluate_ndcg(self.top_k, data.positive_aspect_tag, reason_predict)
        # video_tag
...
class EFMTrainer(Trainer):
    ...
    def train(self, data):  # train mse+bce+l2
        ...
        total_sample = 0
        while True:
            user, item, rating, aspect_tag, aspect_label, video_tag, video_label, interest_tag, interest_label = data.next_batch()
            user = user.to(self.device)  # (batch_size,)
            item = item.to(self.device)
            rating = rating.to(self.device)
            user_aspect_label = self.X_r[user]
            item_aspect_label = self.Y_r[item]
            ...
            self.optimizer.zero_grad()
            rating_loss = self.model.calculate_rating_loss(user, item, rating) * self.rating_weight
            reason_loss = self.model.calculate_reason_loss(user, item, user_aspect_label,
                                                           item_aspect_label) * self.reason_weight
    ...
    def evaluate(self, model, data):  # test
        ...
        with torch.no_grad():
            while True:
                ...
                reason_p = model.rank_aspect_tags(user, item, reason_candidate_tag)  # (batch_size, tag_num)
                _, reason_p_topk = torch.topk(reason_p, dim=-1, k=self.top_k, largest=True,
                                              sorted=True)  # values & index
                reason_predict.extend(reason_candidate_tag.gather(1, reason_p_topk).tolist())
